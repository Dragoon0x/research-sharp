---
id: relational-survey-program
name: Relational Survey Program
domain: rs-survey
version: 1.0.0
---

# Relational Survey Program

**Purpose:** Master relational survey program as a research discipline — the specific methods, frameworks, and judgment calls that produce reliable insights and confident decisions.

Relational Survey Program is where research becomes craft. The difference between a study that produces "interesting findings" and one that changes a $10M product decision is the practitioner's skill in designing the approach, executing the method, and synthesizing the results into action.

## The Core Framework

Effective relational survey program requires three capabilities working together:
...
## The Survey Research Operating System

### The Question Design Discipline
**Clarity test.** Read aloud. Could a respondent interpret two ways? "How often do you use our product?" is ambiguous. "How many times in the past 7 days did you log in?" is not.

**Single-construct rule.** Each question = one thing. "How satisfied with speed AND reliability?" is double-barreled. Split.

**Neutral framing.** "Don't you agree our service is excellent?" leads. "How would you rate our customer service?" is neutral.

**Response scale design.** Agreement: 5 or 7 point. Satisfaction: 1-10 (NPS) or 1-5 (simpler). Label every point. Include "I don't know" when respondents may genuinely lack opinion.

### The Sampling Strategy Framework
**Probability (gold standard).** Every member has known selection chance. Simple random, stratified, cluster. Use when you need population representation.

**Non-probability (practical reality).** Convenience, quota, snowball, purposive. Acknowledge limitations.

**Minimum sizes.** Population estimate ±5% at 95% confidence: ~400. Subgroup analysis: 100 per subgroup. Regression: 10-15 per predictor. NPS ±3%: ~1,000.

### Case Study: Van Westendorp Price Sensitivity
4 questions: too cheap to trust, bargain, expensive but consider, too expensive. Plot cumulative distributions — intersections reveal acceptable price range. Slack used a variant for $8/user/month pricing. Works because it captures psychological pricing boundaries vs unreliable stated willingness-to-pay.

### The Survey Analysis Pipeline
**Step 1 — Cleaning.** Remove speeders (<1/3 median time), straight-liners, bots, incomplete (<60%). Typically removes 5-15%.

**Step 2 — Descriptive.** Frequencies, cross-tabs. What did the population say? How do segments differ?

**Step 3 — Inferential.** Chi-square (categorical), t-tests (means), ANOVA (multi-group), regression (relationships). Is the difference real or noise?

**Step 4 — Driver analysis.** Which factors predict the outcome (satisfaction, purchase intent)? Regression or derived importance. Reveals highest-leverage improvements.

**Step 5 — Storytelling.** Not "here are 47 findings." "Here are 3-5 findings, here's what they mean, here's what to do."

### The Survey Bias Catalog
**Selection bias.** Non-representative respondents. Mitigate: weight responses to match population.

**Response bias.** Socially desirable answers. Mitigate: ask past behavior, not future intent.

**Acquiescence bias.** Agree with everything. Mitigate: mix positive and negative wording.

**Order effects.** Early questions influence later. Mitigate: randomize order.

**Social desirability.** Overreport good, underreport bad. Mitigate: anonymous, indirect techniques.

### The Survey Design Process
**Step 1 — Objectives.** What decisions will the survey inform? 3-5 specific questions the survey must answer.

**Step 2 — Questionnaire draft.** Map each objective to 2-4 questions. Total: 15-25 questions max (completion drops 5-10% per additional question beyond 20).

**Step 3 — Pilot test.** 10-15 respondents from the target population. Time the completion. Identify confusing questions (completion time per question reveals friction). Revise.

**Step 4 — Field.** Distribute through channels that reach the target population. Monitor response rate daily. If <10% after 3 days, adjust incentive or distribution.

**Step 5 — Close and clean.** Close when target sample size is reached or response rate plateaus. Clean data per the pipeline above.

**Step 6 — Analyze and report.** Follow the 5-step analysis pipeline. Present in decision-ready format.

### The NPS Program Design
**Relational NPS** (quarterly): "Overall, how likely are you to recommend [product]?" Measures overall health trend.

**Transactional NPS** (after key interactions): "How likely are you to recommend based on this experience?" Measures specific touchpoint quality.

**Analysis beyond the score.** The score is a symptom. The open-ended "Why?" response is the diagnosis. Code every open-ended response into themes. The themes reveal what drives promoters (protect) and detractors (fix).

**Benchmarking.** Compare to: your own trend (improving or declining?), industry benchmarks (Bain publishes B2B/B2C NPS benchmarks), and competitive NPS (available from some research firms).
...
Getting surveys to the right people:

**Email distribution (highest quality).** Target: known customers or contacts. Expected response rate: 10-30% (higher for engaged users, lower for cold lists). Best for: relational surveys, NPS, customer satisfaction.

**In-app distribution (highest context).** Triggered by specific actions or timing. Expected response rate: 15-40% (users are already engaged). Best for: transactional feedback, feature evaluation, onboarding feedback. Limit to 3-5 questions.

**Panel distribution (highest control).** Third-party panels (Prolific, MTurk, market research panels). Expected response rate: varies by panel and incentive. Best for: market research, concept testing, pricing research. Cost: $2-15 per complete response depending on targeting specificity.

**Social distribution (highest reach, lowest quality).** Posted on social media or forums. Self-selected respondents with unknown demographics. Best for: exploratory research where representativeness isn't critical. Never use for: quantitative claims about a specific population.

### The Advanced Question Types
Beyond basic Likert scales:

**MaxDiff (Best-Worst Scaling).** Show respondents sets of 4-5 items. They pick the best and worst in each set. Produces a preference ranking that's more discriminating than rating scales (where everything gets rated 4/5). Use for: prioritizing features, ranking attributes, understanding relative importance.

**Conjoint.** Respondents evaluate product concepts with different feature/price combinations. Statistical analysis reveals the value (utility) of each feature independently. Use for: pricing optimization, feature prioritization, product configuration decisions. Requires: 200+ respondents and specialized analysis software (Sawtooth, Lighthouse Studio).

**Semantic differential.** Two opposing adjectives with a scale between them: "Simple [1-7] Complex," "Innovative [1-7] Traditional." Captures nuanced perceptions that standard scales miss. Use for: brand perception, product positioning, design evaluation.




### The Continuous Survey Program Design
Moving from ad-hoc surveys to systematic programs:

**Relational surveys (quarterly).** Measure overall relationship health: NPS, satisfaction, brand perception. Same questions every quarter to track trends. Add 2-3 rotating questions per wave to explore specific topics.

**Transactional surveys (triggered).** After key interactions: post-purchase, post-support, post-onboarding. 3-5 questions maximum. Triggered automatically. Measure specific touchpoint quality.

**Pulse surveys (monthly).** 1-2 questions on a current topic. Quick, lightweight, high response rate. Use for: quick reads on new features, reaction to announcements, employee sentiment.

**The program calendar.** Map all survey touchpoints to avoid over-surveying. No customer should receive more than one survey per month. Coordinate across teams — if marketing sends a brand survey on Monday, customer success shouldn't send a satisfaction survey on Wednesday.

### The Survey Reporting Framework
**The 1-3-1 structure.** 1 executive summary slide (the answer, in 30 seconds). 3 insight slides (the evidence, one finding per slide). 1 recommendation slide (what to do, with trade-offs).

**The comparison frame.** Every metric needs context: vs last quarter (trend), vs benchmark (competitive), vs target (performance). A score of 7.2 means nothing in isolation. A score of 7.2 (up from 6.8, above industry benchmark of 6.5, below target of 7.5) tells a complete story.

**The open-ended analysis.** Code every open-ended response into themes. Report the top 5 themes with: frequency (% of responses), representative quotes (3-5 per theme), and sentiment direction (positive, negative, mixed). The open-ends often contain the most actionable insights.
...
When relational survey program is the right method for the research question at hand. When decision stakes are high enough to justify the investment. When existing data is insufficient and new evidence is needed. When the team is operating on assumptions that haven't been validated.
